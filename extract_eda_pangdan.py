#!/usr/bin/env python3
"""EDA 설계운영 인프라 플랫폼 판단서 PDF → Markdown 변환기.

lib.pdf_pipeline.convert_pdf 공통 파이프라인 경유.
Multimodal — 이미지 인라인 상세 분석 포함.
사용법: python3 extract_eda_pangdan.py
...
PDF_PATH = (
    _HERE / "input/pdf"
    / "EDA 설계운영 인프라 플랫폼 추진 방식 선택에 따른 리스크·기회비용·미래가치 판단서.pdf"
)
OUTPUT_DIR = _HERE / "output/pdf_md"

# 이미지 인라인 종합 분석 프롬프트 (verbatim — byte-동일 보존)
PROMPT_TEMPLATE = """PDF 페이지 {start}~{end} 전체를 고품질 Markdown으로 추출하라.
- 페이지 헤더: `## Page N` (N은 실제 PDF 페이지번호, 리셋 금지)
