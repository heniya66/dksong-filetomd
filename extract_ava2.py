#!/usr/bin/env python3
"""AVA_2.pdf → Markdown 변환기 (이미지 위치별 상세 분석 인라인 삽입).

lib.pdf_pipeline.convert_pdf 공통 파이프라인 경유.
대상: <워크스페이스>/input/pdf/AVA_2.pdf
출력: <워크스페이스>/output/pdf_md/AVA_2.md
...
PDF_PATH = _HERE / "input/pdf/AVA_2.pdf"
OUTPUT_DIR = _HERE / "output/pdf_md"
OUTPUT_PATH = OUTPUT_DIR / "AVA_2.md"

# 이미지 위치별 상세 분석 인라인 프롬프트 (verbatim — byte-동일 보존)
...
> • 스크린샷/UI: 화면 영역, UI(User Interface) 컴포넌트, 표시 텍스트·숫자, 사용 맥락
...
>
> **포함된 텍스트/라벨**: 이미지 내부 모든 텍스트·숫자 전사 (OCR(Optical Character Recognition) 수준)
> **본문 연계**: 본문 어느 단락/섹션에서 참조되며 어떤 주장을 뒷받침하는지 1-2문장

추가 규칙:
- 색상 의미(빨강=경고 등) 명시
- 작은 텍스트도 OCR 수준 전사
- 이미지가 없는 페이지는 텍스트만 추출
"""
